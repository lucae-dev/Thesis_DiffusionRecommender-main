...
class SimpleAttentionDiffusionRecommender(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute = None):
        ...
        user_profile_inference = self.diffusion_model.inference(user_profile_batch, self.inference_timesteps)
...
class MultiBlockAttentionDiffusionRecommender(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute = None):
        ...
        if (str(self.device) == 'mps'):
            ...
            sparse_device = self.device        # user_profile_batch = self._dataset.get_batch(user_id_array).to(self.device)
        ...
        if(str(self.device)=="mps"):
            user_profile_batch = user_profile_batch.to("mps")  #Reassign the tensor
...
class SimpleAttentionDiffusionRecommenderOLD(BaseRecommender, Incremental_Training_Early_Stopping, nn.Module):
    """
    Diffusion model based on user profiles, using a single self-attention layer as denoising architecture
    """
