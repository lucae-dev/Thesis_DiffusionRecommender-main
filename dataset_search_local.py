...
if __name__ == '__main__':
    ...
    # Generate the study name
    study_name = model.RECOMMENDER_NAME + '_' + dataset_class()._get_dataset_name()

    # Define the path for the SQLite database file
    sqlite_file = f"{study_name}.db"
    sqlite_url = f"sqlite:///{sqlite_file}"

    # Create the Optuna study with the local SQLite storage
    study = optuna.create_study(study_name=study_name, storage=sqlite_url, load_if_exists=True, direction="maximize")
    ...
    result_df.to_csv(f'{result_table_name}.csv', index=False)

    # Save the DataFrame to a CSV file
    df =  pd.read_csv(filename)
    df.to_csv(f'{study_name}_experiments.csv', index=False)
